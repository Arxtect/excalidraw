---
slug: /@pageflux/excalidraw/api/utils
---

# Utils

These are pure Javascript functions exported from the @pageflux/excalidraw [`@pageflux/excalidraw`](https://npmjs.com/@pageflux/excalidraw). If you want to export your drawings in different formats eg `png`, `svg` and more you can check out [Export Utilities](/docs/@pageflux/excalidraw/API/utils/export). If you want to restore your drawings you can check out [Restore Utilities](/docs/@pageflux/excalidraw/API/utils/restore).
...
```js
import { serializeAsJSON } from "@pageflux/excalidraw";
```
...
```js
import { serializeLibraryAsJSON } from "@pageflux/excalidraw";
```
...
```js
import { isInvisiblySmallElement } from "@pageflux/excalidraw";
```
...
```js
import { loadFromBlob } from "@pageflux/excalidraw";
...
```js
import { loadLibraryFromBlob } from "@pageflux/excalidraw";
```
...
```js showLineNumbers
import { loadSceneOrLibraryFromBlob, MIME_TYPES } from "@pageflux/excalidraw";
...
```js
import { getFreeDrawSvgPath } from "@pageflux/excalidraw";
```
...
```js
import { isLinearElement } from "@pageflux/excalidraw";
```
...
```js
import { getNonDeletedElements } from "@pageflux/excalidraw";
```
...
```js
import { mergeLibraryItems } from "@pageflux/excalidraw";
```
...
```js
import { parseLibraryTokensFromUrl } from "@pageflux/excalidraw";
```
...
```js
import { useHandleLibrary } from "@pageflux/excalidraw";
...
```js
import { getSceneVersion } from "@pageflux/excalidraw";
```
...
```js
import { sceneCoordsToViewportCoords } from "@pageflux/excalidraw";
```
...
```js
import { viewportCoordsToSceneCoords } from "@pageflux/excalidraw";
```
...
```js
import { defaultLang, languages, useI18n } from "@pageflux/excalidraw";
```
...
List of supported language codes. You can pass any of these to `Excalidraw`'s [`langCode` prop](/docs/@pageflux/excalidraw/api/props/#langcode).
...
```js
import { getCommonBounds } from "@pageflux/excalidraw";
```
...
```js
import { elementsOverlappingBBox } from "@pageflux/excalidraw";
```
...
```js
import { isElementInsideBBox } from "@pageflux/excalidraw";
```
...
```js
import { elementPartiallyOverlapsWithOrContainsBBox } from "@pageflux/excalidraw";
```
