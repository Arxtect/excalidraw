...
The need for these two steps is due to the [@pageflux/excalidraw](/docs/@pageflux/excalidraw/installation) being a **UMD** build so we currently cannot import the `convertToExcalidrawElements()` util alone, until we support a tree-shakeable **ESM** build.
...
import { convertToExcalidrawElements}  from "@pageflux/excalidraw"
try {
